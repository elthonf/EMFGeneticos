...
f$min = -500;  #Determina o valor mínimo possível da dimensão (Usado em criação, mutação, etc.)
f$max = +500;  #Determina o valor máximo possível da dimensão (Usado em criação, mutação, etc.)
...
f3.fitness <- function(c){
    return( sum( -c * sin(sqrt(abs(c))) ) );
}

#Cross Over 1: cross de 1 ponto
f3.crossover1 <- function( p1, p2 ){
    return (EMF.Gen.CrossOver.Simple(p1=p1, p2=p2, twoPoints = FALSE));
}

#Cross Over 2: cross uniforme
f3.crossover2 <- function( p1, p2 ){
    return (EMF.Gen.CrossOver.Uniforme(p1=p1, p2=p2));
}

#Mutação 1: Apenas 1 gene tem valor alterado aleatoriamente
f3.mutate1 <- function(original, ... ){
    ret = original;

    #Parte 1: Troca 2 genes de posição
    #Define quais 2 serão alterados
    i1 = sample(1:length(original), size = 2);
    ret[i1[1]] = original[i1[2]];
    ret[i1[2]] = original[i1[1]];

    #Parte 2: Define qual gene será alterado
